    ...
    auth.onAuthStateChanged(async (user) => {
        if (user) {
            ...
            if (snapshot.exists() && snapshot.val() !== sessionKey) {
                alert("Sessão encerrada: Esta conta foi logada em outro dispositivo.");
                auth.signOut(); return;
            }
...
// --- RENDER EPISÓDIOS COM CORREÇÃO DE DATA (FUSO HORÁRIO) ---
window.loadEpisodes = (sNum, eNum = 1) => {
    fetch(`https://api.themoviedb.org/3/tv/${currentMovieData.id}/season/${sNum}?api_key=${API_KEY}&language=pt-BR`).then(r => r.json()).then(d => {
        ...
        document.getElementById('episodeList').innerHTML = d.episodes.map(ep => {
            // FIX DATA: Trata a string da API para evitar erro de fuso
            let date = 'Sem data';
            if (ep.air_date) {
            ...
            return `<button class="ep-btn ${active}" id="ep-btn-${ep.episode_number}" onclick="window.playEpisode(${sNum}, ${ep.episode_number})">
                EP ${ep.episode_number} <br><small style="font-size:0.6rem; opacity:0.7; pointer-events:none;">${date}</small>
            </button>`;
        }).join('');
...
function renderResults(res) {
    ...
    res.forEach(item => {
        ...
        const seasons = item.seasons_count ? `<div class="season-badge" style="pointer-events:none;">${item.seasons_count} Temp.</div>` : '';
        
        let info = ""; 
        if (item.last_ep) {
            const parts = item.last_ep.air_date.split('-');
            const date = `${parts[2]}/${parts[1]}/${parts[0]}`;
            const next = item.next_ep ? `<br>Próx: ${item.next_ep.air_date.split('-').reverse().join('/')}` : '<br>Finalizado';
            info = `<div class="release-info" style="pointer-events:none;">Último: EP ${item.last_ep.episode_number} (${date})${next}</div>`;
        }
        
        card.innerHTML = `<div class="type-badge" style="pointer-events:none;">${label}</div>${seasons}<div class="poster-container"><img src="${poster}">${info}</div><h3 style="pointer-events:none;">${item.title || item.name}</h3>`;
        ...
        if(s.exists()) {
            ...
            arr.forEach(d => {
                ...
                list.innerHTML += `<div class="admin-user-row" style="border-left: 6px solid ${color}; background:#111; padding:20px; display:flex; justify-content:space-between; align-items:center;">
                    <div><b style="font-size:1.1rem; color:#fff">${d.name}</b><br><span style="color:#2ecc71; font-weight:900">R$ ${val.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span></div>
                    <button onclick="window.removeDebt('${d.id}')" style="background:#e74c3c; color:#fff; border:none; padding:8px 12px; border-radius:4px; cursor:pointer; font-weight:bold">DELETAR</button>
                </div>`;
            });
        }
...
function listenToBroadcast() {
    db.ref('admin_broadcast').on('value', (s) => {
        ...
        if(s.exists() && s.val().active) {
            ...
            setTimeout(() => { banner.classList.add('hidden'); if(isAdmin) db.ref('admin_broadcast').update({active: false}); }, 10000);
...
function registerOnline() {
    ...
    db.ref('online_users').on('value', (s) => {
        const list = document.getElementById('onlineList'); if(list) list.innerHTML = Object.values(s.val() || {}).map(u => `<div class="online-user"><span class="online-dot"></span> ${u.name}</div>`).join('');
    });
}
window.loadPlayer = (id, type, title, poster, s=1, e=1) => {
    ...
    if (type === 'tv') {
        document.getElementById('seriesNavigation').classList.remove('hidden'); document.getElementById('nextEpBtn').classList.remove('hidden');
    ...
    } else { document.getElementById('seriesNavigation').classList.add('hidden'); document.getElementById('nextEpBtn').classList.add('hidden'); window.reloadPlayerServer(); saveHistory(id, type, title, poster, 1, 1); }
};
...
window.addDebt = () => { const n = document.getElementById('debtName').value, v = document.getElementById('debtValue').value, p = document.getElementById('debtPriority').value; if(n && v) db.ref('admin_debts').push({name: n, value: v, priority: parseInt(p)}).then(() => { window.showDebtsPage(); }); };
...
window.toggleFavoriteCurrent = () => { const idx = favorites.findIndex(f => f.id === currentMovieData.id); if (idx > -1) favorites.splice(idx, 1); else favorites.unshift(currentMovieData); if(currentUser) db.ref(`users/${currentUser.uid}/favorites`).set(favorites); else localStorage.setItem('gs_favorites', JSON.stringify(favorites)); window.updateFavBtnUI(); };
window.updateFavBtnUI = () => { const btn = document.getElementById('playerFavBtn'); if(btn) btn.innerText = favorites.some(f => f.id === currentMovieData.id) ? "SALVO ❤" : "FAVORITAR"; }
...
window.renderPagination = (total) => { const container = document.getElementById('pagination'); if (currentType === 'favorites' || currentType === 'admin' || total <= 1) { container.innerHTML = ""; return; } let html = `<button class="page-btn" onclick="window.changePage(1)">«</button>`; for (let i = Math.max(1, currentPage-2); i <= Math.min(total, currentPage+2); i++) html += `<button class="page-btn ${i === currentPage ? 'active' : ''}" onclick="window.changePage(${i})">${i}</button>`; container.innerHTML = html + `<button class="page-btn" onclick="window.changePage(${total})">»</button>`; };