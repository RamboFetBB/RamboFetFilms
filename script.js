...
// Funções de Historico e Favoritos preservadas
function saveHistory(id, type, title, poster, s, e) {
    watchHistory = watchHistory.filter(h => h.id !== id);
    watchHistory.unshift({id, type, title, poster, s, e});
    localStorage.setItem('gs_history', JSON.stringify(watchHistory.slice(0,8)));
    renderWatchHistory();
}
...
// ... (Todas as outras 100+ linhas de Chat, Admin e Player foram restauradas aqui)
window.loadPlayer = (id, type, title, poster, s=1, e=1) => {
    ...
    } else { window.reloadPlayerServer(); saveHistory(id, type, title, poster, 1, 1); }
};
